...
void borneraMacho(char cablePositivo[], char cableNegativo[]) {
    int correcto = 1;

    if (strcmp(cablePositivo, "rojo") != 0) {
        printf("El cable conectado al pin positivo debe ser de color rojo\n");
        correcto = 0;
    }

    if (strcmp(cableNegativo, "negro") != 0) {
        printf("El cable conectado al pin negativo debe ser de color negro\n");
        correcto = 0;
    }

    if (correcto) {
        printf("Conexión correcta:\n");
        printf("Pin positivo: %s\n", cablePositivo);
        printf("Pin negativo: %s\n", cableNegativo);
    } else {
...
int main() {
  ...
  methods: {
    ...
    finish() {
      ...
      router.push('/GeneralizacionConectarFuentePoderBorneraMacho').then(() => {
        window.scrollTo(0, 0);
      });
    }
  }
